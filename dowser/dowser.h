...
// Correct for placeWat.c
int PairList(REAL *ohh, int *NBPairs, int Num_atoms, PDB *pro);
void Align(int n, REAL *water, int start, float *bisect, float *e_grad);
void TurnMol(float angle, float *bisect, REAL *water, int start, int n);

void readPDB10(char *filename, int *numAtms, PDB **pro);
int SetAmideFlag(PDB *pro);
int GroupAmides(void);
int extbit(int string, int bit);
void initVDW(char *filename, int *numAtms, PDB **pro);
